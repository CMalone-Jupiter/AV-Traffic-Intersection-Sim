...
class AVState(pomdp_py.State):
    """
    POMDP state:
      - ego_region: where the AV is relative to the intersection
      - ego_speed: qualitative speed state
      - unseen_left / unseen_right: how dangerous unseen cars are from each side
    """

    REGIONS = ["before_edge", "at_edge", "in_intersection"]
    SPEEDS = ["stopped", "creeping", "going"]
    UNSEEN_LEVELS = ["none", "far", "near"]
    ...
    def __init__(self,
                 ...
                 unseen_left: str,
                 unseen_right: str):
        if ego_region not in self.REGIONS:
            raise ValueError(f"Invalid ego_region: {ego_region}")
        if ego_speed not in self.SPEEDS:
            raise ValueError(f"Invalid ego_speed: {ego_speed}")
        if unseen_left not in self.UNSEEN_LEVELS:
            raise ValueError(f"Invalid unseen_left: {unseen_left}")
        if unseen_right not in self.UNSEEN_LEVELS:
            raise ValueError(f"Invalid unseen_right: {unseen_right}")

        self.ego_region = ego_region
        self.ego_speed = ego_speed
        self.unseen_left = unseen_left
        self.unseen_right = unseen_right

    def __hash__(self):
        return hash((self.ego_region,
                     self.ego_speed,
                     self.unseen_left,
                     self.unseen_right))

    def __eq__(self, other):
        return (isinstance(other, AVState)
                and self.ego_region == other.ego_region
                and self.ego_speed == other.ego_speed
                and self.unseen_left == other.unseen_left
                and self.unseen_right == other.unseen_right)

    def __str__(self):
        return (f"AVState(region={self.ego_region}, speed={self.ego_speed}, "
                f"L={self.unseen_left}, R={self.unseen_right})")

    # ---- helper: map to coarse danger level for observation / reward ----
    def danger_level(self) -> str:
        """
        Map unseen_left/right into a coarse danger label:
         - 'high'   if any side has NEAR unseen car
         - 'medium' if any side has FAR unseen car (and none NEAR)
         - 'low'    otherwise
        """
        if self.unseen_left == "near" or self.unseen_right == "near":
            return "high"
        if self.unseen_left == "far" or self.unseen_right == "far":
            return "medium"
        return "low"
...
class OcclusionObservation(pomdp_py.Observation):
    ...
    def __init__(self, reading: str):
        # Observations are coarse danger labels: 'low', 'medium', 'high'
        if reading not in ["low", "medium", "high"]:
...
class TransitionModel(pomdp_py.TransitionModel):
    ...
    def __init__(self, config: UnseenCarPOMDPConfig = POMDP_CONFIG):
        self.config = config
        self._build_unseen_transition()

    def _build_unseen_transition(self):
        """
        Transition for unseen_left / unseen_right per side.
        Interpreted as how the 'unseen danger' evolves as AV gathers info.
        """

        cfg = self.config

        # Base natural change (if AV stops)
        # near -> far/near, far -> none/far, none -> none
        nat = cfg.natural_change_rate

        stop_unseen = {
            "near":  {"near": 1.0 - nat, "far": nat * 0.7, "none": nat * 0.3},
            "far":   {"near": 0.0,       "far": 1.0 - nat, "none": nat},
            "none":  {"near": 0.0,       "far": 0.0,       "none": 1.0}
        }

      
        imp = cfg.creep_improvement_rate
        creep_unseen = {
            "near":  {"near": 1.0 - imp, "far": imp * 0.6, "none": imp * 0.4},
            "far":   {"near": 0.0,       "far": 1.0 - imp, "none": imp},
            "none":  {"near": 0.0,       "far": 0.0,       "none": 1.0}
        }

       
        go_unseen = {
            "near":  {"near": 1.0, "far": 0.0, "none": 0.0},
            "far":   {"near": 0.0, "far": 1.0, "none": 0.0},
            "none":  {"near": 0.0, "far": 0.0, "none": 1.0}
        }
    ...
    # ---- deterministic transitions for ego_region and ego_speed ----

    def _next_region_det(self, region: str, action: AVAction) -> str:
        """
        Simple deterministic region transition:
          - stop: stay
          - creep: move towards intersection
          - go: end up inside intersection
        """
        if action.name == "stop":
            return region

        if action.name == "creep":
            if region == "before_edge":
                return "at_edge"
            elif region == "at_edge":
    ...
    def _next_speed_det(self, action: AVAction) -> str:
        """Deterministic speed state based on chosen action."""
        if action.name == "stop":
            return "stopped"
    ...
    def _unseen_single_prob(self, next_level: str, level: str, action: AVAction) -> float:
        return self.unseen_trans[action.name][level][next_level]

    def probability(self, next_state: AVState, state: AVState, action: AVAction) -> float:
        # 1) region transition
        desired_region = self._next_region_det(state.ego_region, action)
        p_region = 1.0 if next_state.ego_region == desired_region else 0.0

        # 2) speed transition
        desired_speed = self._next_speed_det(action)
        p_speed = 1.0 if next_state.ego_speed == desired_speed else 0.0

        if p_region == 0.0 or p_speed == 0.0:
            return 0.0

        # 3) unseen per side
        p_unseen_left = self._unseen_single_prob(
            next_state.unseen_left, state.unseen_left, action
        )
        p_unseen_right = self._unseen_single_prob(
            next_state.unseen_right, state.unseen_right, action
        )

        return p_region * p_speed * p_unseen_left * p_unseen_right

    def sample(self, state: AVState, action: AVAction) -> AVState:
        # deterministic part
        next_region = self._next_region_det(state.ego_region, action)
        next_speed = self._next_speed_det(action)

        # stochastic unseen_left
        def sample_unseen(current_level: str) -> str:
            probs = self.unseen_trans[action.name][current_level]
            r = random.random()
            cumsum = 0.0
        ...
        next_unseen_left = sample_unseen(state.unseen_left)
        next_unseen_right = sample_unseen(state.unseen_right)

        return AVState(next_region, next_speed, next_unseen_left, next_unseen_right)
...
class ObservationModel(pomdp_py.ObservationModel):
    ...
    def probability(self,
                    observation: OcclusionObservation,
                    next_state: AVState,
                    action: AVAction) -> float:
        """
        Sensor gives a coarse reading of 'low/medium/high' based on true
        danger level derived from unseen_left/right in AVState.
        """
        true_level = next_state.danger_level()   # 'low' / 'medium' / 'high'
        obs_level = observation.reading          # same vocabulary
    ...
    def sample(self, next_state: AVState, action: AVAction) -> OcclusionObservation:
        ...
        # fallback
        return OcclusionObservation(next_state.danger_level())
...
class RewardModel(pomdp_py.RewardModel):
    ...
    def _reward(self, state: AVState, action: AVAction, next_state: AVState) -> float:
        """
        Use:
          - small costs for stop/creep
          - large positive/negative reward for GO, based on danger level
          - extra penalty for creeping / stopping inside intersection
        """
        cfg = self.config

        # cost for staying stopped
        if action.name == "stop":
            base = cfg.cost_stop
            # penalize stopping inside intersection more
            if state.ego_region == "in_intersection":
                base -= 10.0
            return base
        ...
        # GO
        if action.name == "go":
            # discourage 'go' when not at edge 
            if state.ego_region != "at_edge":
                # strong penalty if going from before_edge or already in intersection
                return cfg.reward_go_high  

            danger = state.danger_level()  # 'low', 'medium', 'high'
            if danger == "low":
                return cfg.reward_go_low
            elif danger == "medium":
                return cfg.reward_go_medium
            else:
                return cfg.reward_go_high

        return 0.0

    def sample(self, state: AVState, action: AVAction, next_state: AVState) -> float:
...
class AVPolicy(pomdp_py.RolloutPolicy):
    ...
    def get_belief_summary(self, belief: pomdp_py.Histogram) -> str:
        ...
        if total == 0:
            return "P(H:0.00 M:0.00 L:0.00)"

        prob_high = prob_med = prob_low = 0.0
        for state, p in hist.items():
            if not isinstance(state, AVState):
                continue
            d = state.danger_level()
            if d == "high":
                prob_high += p
            elif d == "medium":
                prob_med += p
    ...
    def compute_expected_reward(self, belief: pomdp_py.Histogram, action: AVAction) -> float:
        """
        Expected immediate reward under current belief and chosen action.
        Sum over current belief support for both current and next states.
        """
        hist = belief.get_histogram()
        expected_reward = 0.0

        # use the same support for next_state as current belief (small state space)
        possible_next_states = list(hist.keys())
...
class UnseenCarPOMDPAgent:
    ...
    def __init__(self, config: UnseenCarPOMDPConfig = POMDP_CONFIG):
        ...
        # Initialize belief
        # Start belief: AV before edge, stopped, and no strong belief of unseen cars
        initial_belief = {}

        # For simplicity, uniform over unseen_left/right ∈ {none, far}
        unseen_levels = ["none", "far"]
        num_states = len(unseen_levels) ** 2  # left × right

        for l in unseen_levels:
            for r in unseen_levels:
                s = AVState(
                    ego_region="before_edge",
                    ego_speed="stopped",
                    unseen_left=l,
                    unseen_right=r
                )
                initial_belief[s] = 1.0 / num_states
    ...
    def update_belief(self, observation_str: str):
        """Bayesian belief update over AVState belief support."""
        observation = OcclusionObservation(observation_str)
        current_belief = self.belief.get_histogram()
        new_belief: Dict[AVState, float] = {}

        # treat the current belief support as the set of possible next states
        possible_next_states = list(current_belief.keys())

        for next_state in possible_next_states:
            # likelihood of observation given next_state
            obs_prob = self.observation_model.probability(
                observation, next_state, self.last_action
            )
            if obs_prob == 0.0:
                continue
            ...
            for state, belief_weight in current_belief.items():
                trans_prob = self.transition_model.probability(
                    next_state, state, self.last_action
                )
        ...
        if total > 0.0:
            ...
            # fallback: keep previous belief if something degenerated
            self.belief = pomdp_py.Histogram(current_belief)
